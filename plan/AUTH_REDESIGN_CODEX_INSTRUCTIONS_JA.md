...
- 実装とあわせて自動テストも追加すること。
- test 関係のファイルは `tests/` ディレクトリへ集約すること。
- ローカル開発環境はクローズドな環境でも実行可能な構成を維持すること。
- ローカル開発時の認証シミュレータは、実現可能な範囲で `Magnito` を優先して使用すること。
...
実装は、外部に接続できないローカル開発環境でも成立すること。

- クローズドまたは分離されたローカル環境でも認証フローを実行可能にすること。
- 必要な挙動を表現できる限り、ローカルの認証シミュレータとして `Magnito` を優先すること。
- 通常のローカル検証において、クラウド依存でなければ成立しない設計にしないこと。
- 将来的に Passkey 関連で `Magnito` だけでは再現できない部分があっても、少なくともベースログインフローと TOTP step-up はローカル完結で動作可能にすること。
...
認証再設計の実装とあわせて、自動テストも整備すること。

- test 関係のファイルは `tests/` ディレクトリ配下に配置すること。
- 認証再設計後のフローが、通常のローカル開発フローの中で検証可能になるようにテストを追加または拡張すること。
- 少なくとも、認可開始、callback 検証、session 発行、session 状態照会、TOTP による step-up を確認できるようにすること。
- テストもクローズドなローカル環境前提に合わせ、可能な限り `Magnito` を使ったローカル挙動を前提にすること。
- 強い既存慣習がない限り、新しい認証系テストファイルを `tests/` 以外へ分散させないこと。
...
- 認証再設計に対応した自動テストを追加すること
- 認証関連のテストを `tests/` ディレクトリ配下に整理すること
- クローズドなローカル開発環境でも実行可能であり、可能な限り `Magnito` を利用すること
